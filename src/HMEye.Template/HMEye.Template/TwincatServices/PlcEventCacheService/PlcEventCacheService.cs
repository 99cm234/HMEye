...
namespace HMEye.TwincatServices
{
	...
	/// </summary>
	public class PlcEventCacheService : IPlcEventCacheService, IHostedService
	{
		private readonly IEventLoggerService _eventLogger;
		private readonly ILogger<PlcEventCacheService> _logger;
		private readonly PlcEventCacheSettings _settings;
		private readonly CancellationTokenSource _cts = new();
		private readonly object _lock = new();
		private Timer? _alarmRefreshTimer;
		private Timer? _eventRefreshTimer;
		private ImmutableList<EventLoggerEvent> _eventsCache = ImmutableList<EventLoggerEvent>.Empty;
		private ImmutableList<EventLoggerEvent> _activeAlarmsCache = ImmutableList<EventLoggerEvent>.Empty;
		private bool _error;
		...
		public PlcEventCacheService(
			IEventLoggerService eventLogger,
			ILogger<PlcEventCacheService> logger,
			IOptions<PlcEventCacheSettings> options)
		{
		...
		public IReadOnlyCollection<EventLoggerEvent> GetCachedEvents() => _eventsCache;

		public IReadOnlyCollection<EventLoggerEvent> GetCachedActiveAlarms() => _activeAlarmsCache;
