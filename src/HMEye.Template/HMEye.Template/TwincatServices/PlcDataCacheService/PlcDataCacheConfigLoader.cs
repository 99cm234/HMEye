...
public class PlcDataCacheConfigLoader : IDisposable
{
	private readonly ILogger<PlcDataCacheConfigLoader> _logger;
	...
	public PlcDataCacheConfigLoader(ILogger<PlcDataCacheConfigLoader> logger, IOptions<TwincatSettings> options)
	{
	...
	/// <returns>A task that resolves to an <see cref="IEnumerable{T}"/> of <see cref="CacheItemConfig"/> objects representing the cacheable symbols.</returns>
	/// <exception cref="Exception">Thrown if symbol initialization fails or symbols are not loaded.</exception>
	public async Task<IEnumerable<CacheItemConfig>> CreateCacheItemConfigs(
	...
	)
	{
		...
		using (timeoutCts)
		{
			...
			var configs = new List<CacheItemConfig>();
	...
	/// </remarks>
	private IEnumerable<CacheItemConfig> ScanSymbolRecursive(ISymbol symbol, CancellationToken cancellationToken)
	{
		var results = new List<CacheItemConfig>();
		cancellationToken.ThrowIfCancellationRequested();
	...
	private static bool TryResolveByName(string? name, out Type? type)
	{
		if (name is not null)
		{
			if (TwincatTypeMaps.Map.TryGetValue(name, out type))
				return true;

			if (TwincatCustomTypeMaps.Map.TryGetValue(name, out type))
				return true;
		}
		type = null;
		return false;
