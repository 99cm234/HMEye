...
namespace HMEye.TwincatServices.Endpoints;

public static class TwincatDataEndpoints
{
	public static void MapTwincatDataEndpoints(this IEndpointRouteBuilder app)
	{
		var group = app.MapGroup("/api/plc-data").RequireAuthorization("ApiKeyOrAuthenticated");

		group.MapGet(
			"/{type}/{address}",
			(string type, string address, [FromServices] ITwincatCache dataCache) =>
			{
		...
		group.MapPost(
			"/bulk-read",
			(BulkRequest request, [FromServices] ITwincatCache dataCache) =>
			{
		...
		group.MapPost(
			"/{type}/{address}",
			(string type, string address, [FromBody] JsonElement body, [FromServices] ITwincatCache dataCache) =>
			{
	...
	private static (object? Value, string? Error) ResolveRead(ITwincatCache dataCache, string type, string address)
	{
	...
	private static (object? Value, string? Error) ReadValue<T>(ITwincatCache dataCache, string address)
	{
	...
	private static (bool Success, string? Error) WriteValue<T>(ITwincatCache dataCache, string address, JsonElement body)
		where T : notnull
	{
		T value;
		try
