...
using HMEye.TwincatServices.Contracts.TypeMaps;
...
public class TwincatCacheConfigLoader : IDisposable
{
	private readonly ILogger<TwincatCacheConfigLoader> _logger;
	...
	public TwincatCacheConfigLoader(ILogger<TwincatCacheConfigLoader> logger, IOptions<TwincatSettings> options)
	{
	...
	private bool EnsureConnected()
	{
		if (!_adsClient.IsConnected && _netId != null)
		{
			try
			{
				...
				_logger.LogInformation("TwincatCacheConfigLoader successfully connected TwinCAT PLC AdsClient.");
				return true;
			}
			catch (Exception ex)
			{
				_logger.LogError(ex, "TwincatCacheConfigLoader failed to connect to TwinCAT PLC AdsClient.");
				throw;
			}
		}
		return true;
	...
	private async Task EnsureSymbolLoaderInitializedAsync(CancellationToken cancellationToken = default)
	{
		...
		_symbols =
			...
			?? throw new Exception("TwincatCacheConfigLoader failed to initialize symbols.");
	}
	...
	/// <returns>A task that resolves to an <see cref="IEnumerable{T}"/> of <see cref="TwincatCacheItemConfig"/> objects representing the cacheable symbols.</returns>
	/// <exception cref="Exception">Thrown if symbol initialization fails or symbols are not loaded.</exception>
	public async Task<IEnumerable<TwincatCacheItemConfig>> CreateCacheItemConfigs(
	...
	)
	{
		...
		using (timeoutCts)
		{
			...
			var configs = new List<TwincatCacheItemConfig>();
	...
	/// </remarks>
	private IEnumerable<TwincatCacheItemConfig> ScanSymbolRecursive(ISymbol symbol, CancellationToken cancellationToken)
	{
		var results = new List<TwincatCacheItemConfig>();
		cancellationToken.ThrowIfCancellationRequested();
