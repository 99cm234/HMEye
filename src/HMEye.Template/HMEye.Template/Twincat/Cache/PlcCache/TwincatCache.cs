...
{
		public class TwincatCache : ITwincatCache, IHostedService, IDisposable
	{
		private readonly ITwincatService _plcService;
		private readonly ILogger<TwincatCache> _logger;
		private readonly ConcurrentDictionary<string, ITwincatCacheItem> _cacheItems;
		private readonly ConcurrentQueue<ITwincatCacheWriteOperation> _writeQueue = new();
		...
		public TwincatCache(ITwincatService plcService, ILogger<TwincatCache> logger, IEnumerable<TwincatCacheItemConfig>? configs)
		{
			_plcService = plcService ?? throw new ArgumentNullException(nameof(plcService));
			_logger = logger ?? throw new ArgumentNullException(nameof(logger));

			_cacheItems = new ConcurrentDictionary<string, ITwincatCacheItem>(
				(configs ?? Enumerable.Empty<TwincatCacheItemConfig>()).ToDictionary(
					cfg => cfg.Address,
		...
		private static ITwincatCacheItem CreateCacheItem(TwincatCacheItemConfig config, ITwincatService plcService)
		{
			if (config.IsDynamic)
			{
				var cacheItemType = typeof(TwincatCacheItemDynamic<>).MakeGenericType(config.Type);
				return (ITwincatCacheItem)
			...
			{
				var cacheItemType = typeof(TwincatCacheItem<>).MakeGenericType(config.Type);
				return (ITwincatCacheItem)
					Activator.CreateInstance(
						cacheItemType,
						config.Address,
		...
		public async Task<TwincatCacheReadResult<T>> TryReadImmediateAsync<T>(string address)
		{
			if (_cacheItems.TryGetValue(address, out var item) && item.Type == typeof(T))
			{
				...
				{
					await item.GetAsync();
					return new TwincatCacheReadResult<T>((T)item.GetValue()!);
				}
				catch (Exception ex)
				{
					SetError($"Failed to read {address}", ex);
					return new TwincatCacheReadResult<T>(default, true, ex.Message);
				}
			}
			return new TwincatCacheReadResult<T>(default, true, $"Variable {address} not found or type mismatch");
		}
		...
		public TwincatCacheReadResult<T> TryReadCached<T>(string address)
		{
			if (_cacheItems.TryGetValue(address, out var item) && item.Type == typeof(T))
			{
				return new TwincatCacheReadResult<T>((T)item.GetValue()!);
		...
		public TwincatCacheWriteResult TryWriteQueue<T>(string address, T value)
			where T : notnull
		{
			if (_cacheItems.TryGetValue(address, out var item) && item.Type.IsAssignableFrom(typeof(T)))
			{
				if (item.IsReadOnly)
					return new TwincatCacheWriteResult(false, $"Variable {address} is read-only");

				var operation = item.CreateWriteOperation(value);
				if (_writeQueue.Count < 100)
				{
					_writeQueue.Enqueue(operation);
					return new TwincatCacheWriteResult(true);
				}
				else
				{
					...
					return new TwincatCacheWriteResult(false, errorMsg);
				}
			}
			return new TwincatCacheWriteResult(false, $"Variable {address} not found or type mismatch");
		}
		...
		public async Task<TwincatCacheWriteResult> TryWriteImmediateAsync<T>(string address, T value)
			where T : notnull
		{
			if (_cacheItems.TryGetValue(address, out var item) && item.Type.IsAssignableFrom(typeof(T)))
			{
				...
				catch (Exception ex)
				{
					...
					return new TwincatCacheWriteResult(false, ex.Message);
				}
			}
			return new TwincatCacheWriteResult(false, $"Variable {address} not found or type mismatch");
		}
		...
		public void AddCacheItem(TwincatCacheItemConfig config)
		{
			if (config == null)
			{
