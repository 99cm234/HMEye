...
namespace HMEye.TwincatServices
{
	...
	/// </code>
	/// </remarks>
	public interface IEventLoggerService : IDisposable, IHostedService
	{
		/// <summary>
		...
		/// <returns>A task that resolves to a list of EventLogger events converted to <see cref="EventLoggerEvent"/> type.</returns>
		...
		Task<List<EventLoggerEvent>> GetEventLogsAsync(uint nMaxEvents = 50, CancellationToken cancellationToken = default);

		/// <summary>
		/// Gets all active EventLogger alarms.
		/// </summary>
		/// <param name="cancellationToken">The cancellation token to cancel the operation.</param>
		/// <returns>A task that resolves to a list of active EventLogger alarms converted to <see cref="EventLoggerEvent"/> type.</returns>
		/// <exception cref="AdsErrorException">Thrown when an ADS-specific error occurs.</exception>
		/// <exception cref="TimeoutException">Thrown when the operation times out.</exception>
		/// <exception cref="OperationCanceledException">Thrown when the operation is canceled.</exception>
		/// <exception cref="Exception">Thrown when an unexpected error occurs.</exception>
		Task<List<EventLoggerEvent>> GetActiveAlarmsAsync(CancellationToken cancellationToken = default);
