...
namespace HMEye.TwincatServices
{
    public class PlcService : IDisposable, IHostedService, IPlcService
    {
        ...
        public PlcService(
            ...
            IOptions<TwincatSettings> options
        )
        {
        ...
        private async Task EnsureSymbolLoaderInitializedAsync(
        ...
        public async Task<ResultReadDeviceState> ReadDeviceStateAsync(
            CancellationToken cancellationToken = default
        )
        {
            ...
            using (timeoutCts)
            {
                ...
                catch (OperationCanceledException canceledEx)
                {
                    _logger.LogError(
                        canceledEx,
                        "Operation was canceled while reading device state"
                    );
                    throw;
                }
                catch (Exception ex)
        ...
        )
        {
            ...
            using (timeoutCts)
            {
                ...
                catch (OperationCanceledException canceledEx)
                {
                    _logger.LogError(
                        canceledEx,
                        "Operation was canceled while reading device info"
                    );
                    throw;
                }
                catch (Exception ex)
        ...
        public async Task PlcCommandAsync(
            AdsState adsState,
            CancellationToken cancellationToken = default
        )
        {
            ...
            using (timeoutCts)
            {
                ...
                catch (OperationCanceledException canceledEx)
                {
                    _logger.LogError(
                        canceledEx,
                        "Operation was canceled while issuing PLC command"
                    );
                    throw;
                }
                catch (Exception ex)
        ...
        public async Task<T> ReadVariableAsync<T>(
            string variableName,
            CancellationToken cancellationToken = default
        )
        {
            ...
            using (timeoutCts)
            {
                ...
                try
                {
                    ...
                    {
                        _logger.LogError(
                            "Symbol '{variableName}' not found or not a DynamicSymbol.",
                            variableName
                        );
                    ...
                    {
                        _logger.LogError(
                            "Failed to read dynamic value for variable: {VariableName}",
                            variableName
                        );
                    ...
                    _logger.LogInformation(
                        "Successfully read dynamic value for variable: {VariableName}",
                        variableName
                    );
                ...
                catch (AdsErrorException adsEx)
                {
                    _logger.LogError(
                        adsEx,
                        "ADS error occurred while reading symbol: '{variableName}'",
                        variableName
                    );
                    throw;
                }
        ...
        public async Task WriteVariableAsync<T>(
            string variableName,
            T value,
            CancellationToken cancellationToken = default
        )
        {
            ...
            using (timeoutCts)
            {
                ...
                try
                {
                    ...
                    {
                        _logger.LogError(
                            "Value for variable '{variableName}' is null.",
                            variableName
                        );
                        throw new ArgumentNullException(
                            nameof(value),
                            $"Value for variable '{variableName}' cannot be null."
                        );
                    }
                    ...
                    if (typeof(T) == typeof(string))
                    {
                        ...
                        {
                            throw new InvalidOperationException(
                                $"DataType is null for symbol '{variableName}'."
                            );
                        }
                        int maxLength = symbol.DataType.Size;
                        string stringValue = value.ToString() ?? string.Empty;
                        var encoding =
                            symbol.ValueEncoding == Encoding.UTF8 ? Encoding.UTF8 : Encoding.Latin1;
                    ...
                    _logger.LogInformation(
                        "Successfully wrote dynamic value for variable: {VariableName}",
                        variableName
                    );
                }
                catch (AdsErrorException adsEx)
                {
                    _logger.LogError(
                        adsEx,
                        "ADS error occurred while writing symbol: '{variableName}'",
                        variableName
                    );
                    throw;
                }
        ...
        )
        {
            ...
            using (timeoutCts)
            {
                ...
                try
                {
                    ...
                    if (_symbols[arrayName] is not DynamicArrayInstance arrayInstance)
                        throw new KeyNotFoundException(
                            $"Symbol '{arrayName}' not found or not an array."
                        );

                    if (arrayInstance.DataType?.Category != DataTypeCategory.Array)
                        throw new InvalidOperationException(
                            $"Symbol '{arrayName}' is not an array."
                        );

                    var result = await arrayInstance.ReadValueAsync(linkedCts.Token);
                    if (!result.Succeeded)
                        throw new InvalidOperationException(
                            $"Failed to read values from array '{arrayName}': {result.ErrorCode}"
                        );

                    if (result.Value is Array multiDimArray)
                        return multiDimArray.Cast<T>().ToArray();

                    if (result.Value is T[] arrayValues)
                        return arrayValues;

                    throw new InvalidOperationException(
                        $"Unexpected array type for '{arrayName}'."
                    );
                }
                catch (AdsErrorException adsEx)
                {
                    _logger.LogError(
                        adsEx,
                        "ADS error occurred while reading array: '{arrayName}'",
                        arrayName
                    );
                    throw;
                }
                catch (TimeoutException timeoutEx)
                {
                    _logger.LogError(
                        timeoutEx,
                        "Timeout occurred while reading array: '{arrayName}'",
                        arrayName
                    );
                    throw;
                }
                catch (OperationCanceledException canceledEx)
                {
                    _logger.LogError(
                        canceledEx,
                        "Operation was canceled while reading symbol: '{arrayName}'",
                        arrayName
                    );
                    throw;
                }
                ...
                {
                    _logger.LogError(ex, "Failed to read array: '{arrayName}'", arrayName);
                    throw;
                }
                finally
                {
        ...
        public async Task WriteArrayAsync<T>(
            string arrayName,
            T[] values,
            CancellationToken cancellationToken = default
        )
            ...
            using (timeoutCts)
            {
                ...
                try
                {
                    ...
                    if (values.Length != totalElements)
                        throw new ArgumentException(
                            $"Array size mismatch: Expected {totalElements}, got {values.Length}."
                        );
                    ...
                    if (typeof(T) == typeof(string))
                    {
                        if (arrayInstance.ElementType == null)
                            throw new InvalidOperationException(
                                $"Element type or data type is null for array '{arrayName}'."
                            );

                        int targetMaxLength = arrayInstance.ElementType.Size;
                        var encoding =
                            arrayInstance.ValueEncoding == Encoding.UTF8
                                ? Encoding.UTF8
                                : Encoding.Latin1;

                        foreach (
                            var (value, index) in values.Select((value, index) => (value, index))
                        )
                        {
                            string stringValue = value?.ToString() ?? string.Empty;
                            var byteCount = encoding.GetByteCount(stringValue);
                            ...
                            {
                                throw new ArgumentException(
                                    $"String at index {index} exceeds the maximum allowed length of {targetMaxLength - 1} characters."
                                );
                            }
                        }
                    }

                    var result = await arrayInstance.WriteValueAsync(values, linkedCts.Token);
                    if (!result.Succeeded)
                        throw new InvalidOperationException(
                            $"Failed to write values to array '{arrayName}': {result.ErrorCode}"
                        );
                }
                catch (AdsErrorException adsEx)
                {
                    _logger.LogError(
                        adsEx,
                        "ADS error occurred while writing array: '{arrayName}'",
                        arrayName
                    );
                    throw;
                }
                catch (TimeoutException timeoutEx)
                {
                    _logger.LogError(
                        timeoutEx,
                        "Timeout occurred while writing array: '{arrayName}'",
                        arrayName
                    );
                    throw;
                }
                catch (OperationCanceledException canceledEx)
                {
                    _logger.LogError(
                        canceledEx,
                        "Operation was canceled while writing symbol: '{arrayName}'",
                        arrayName
                    );
                    throw;
                }
                ...
                {
                    _logger.LogError(ex, "Failed to write array: '{arrayName}'", arrayName);
                    throw;
                }
                finally
                {
