...
{
	public class EventLoggerService : IDisposable, IHostedService, IEventLoggerService
	{
		...
		public EventLoggerService(ILogger<EventLoggerService> logger, IOptions<TwincatSettings> options)
		{
		...
		public async Task<List<EventLoggerEvent>> GetEventLogsAsync(
			uint nMaxEvents = 50,
		...
		/// <summary>
		/// Converts a collection of logged events to a list of <see cref="EventLoggerEvent"/> objects.
		/// </summary>
		/// <param name="eventCollection">The collection of logged events to convert.</param>
		/// <returns>A list of converted <see cref="EventLoggerEvent"/> objects.</returns>
		/// <exception cref="ArgumentNullException">Thrown when <paramref name="eventCollection"/> is null.</exception>
		private static List<EventLoggerEvent> ConvertToLocalEvents(ITcLoggedEventCollection eventCollection)
		{
			ArgumentNullException.ThrowIfNull(eventCollection);

			return eventCollection
				.OfType<TcLoggedEvent>()
				.Select(x => new EventLoggerEvent
				{
		...
		/// <summary>
		/// Converts a collection of alarms to a list of <see cref="EventLoggerEvent"/> objects.
		/// </summary>
		...
		private static List<EventLoggerEvent> ConvertToLocalEvents(ITcAlarmCollection alarmCollection)
		{
		...
		public async Task<List<EventLoggerEvent>> GetActiveAlarmsAsync(CancellationToken cancellationToken = default)
		{
			var (linkedCts, timeoutCts) = CreateLinkedCancellationTokenSource(cancellationToken);
			using (linkedCts)
			using (timeoutCts)
